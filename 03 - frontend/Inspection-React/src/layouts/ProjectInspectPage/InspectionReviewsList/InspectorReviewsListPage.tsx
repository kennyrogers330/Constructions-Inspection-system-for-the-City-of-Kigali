...
export const InspectorReviewsListPage = () => {
    ...
    useEffect(() => {
        const fetchProjectReviews = async () => {
           const reviewUrl: string = `http://localhost:8080/api/inspectorReviews/search/findByProjectId?projectId=${projectId}&page=${currentPage - 1}&size=${reviewsPerPage}`;
    ...
    return(
        <div className="container m-5">
            ...
            <div className="row">
                {reviews.map(review =>(
                    <InspectorReview review={review} key={review.id}/>
                ))}
            </div>
